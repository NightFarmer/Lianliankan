...
var pathUtil = {
		...
		findPath: function (start, end, map, marker) {
			var self = this;
			var open = [];
			var close = [];
			...
			console.info(marker)
			var _getNextPath = function(prePath, nodeEnd){
				...
				for (var i = latestNode.x - 1; i <= latestNode.x+1; i++) {//添加备选节点
					for (var j = latestNode.y - 1; j <= latestNode.y+1; j++) {
						...
						if(!(map[i] && map[i][j] && map[i][j] == marker) && !(i === nodeEnd.x && j ===nodeEnd.y)){//如果是障碍物，或在close列表中，且不是目标
							continue;
						}
			...
			for(var i=0; i<ways.length;i++){
				for(var j=i+1; j+1<ways.length;j++){
					if(ways[i].x==ways[j].x && ways[j].x==ways[j+1].x ||
						ways[i].y==ways[j].y && ways[j].y==ways[j+1].y ||
						Math.abs(ways[i].x-ways[j].x)==Math.abs(ways[i].y-ways[j].y) && Math.abs(ways[j].x-ways[j+1].x)==Math.abs(ways[j].y-ways[j+1].y)
					){
						ways.splice(i+1,1)
						j--;
					}
